...
contact_inds = logical(data_(:,20)); 
contact_transitions = diff(data_(:,20));
ATI_offsets_inds = contact_transitions==1;

%have to add on another transition for the end
% ATI_offsets_inds = [ATI_offsets_inds ; 0];


% iterate through data and offset ATI forces
%group length = 10x group length in ellipsoid_training_traj code (because 10
%timesteps at every point from generated traj)
transition_inds = find(ATI_offsets_inds==1,2);
gl = transition_inds(2) - transition_inds(1); 

gl_new = length(ATI_offsets_inds) - gl + 20 +1;
for ii=1:length(ATI_offsets_inds)
    %*************special case for fA4 new data*****************%
    if (ii == gl_new)
        data_(ii+1:ii+gl-20-1,2:4) = data_(ii+1:ii+gl-20-1,2:4) - data_(ii,2:4);
    
    %***********************************************************%
    elseif (ATI_offsets_inds(ii)==1)
        data_(ii+1:ii+gl-1,2:4) = data_(ii+1:ii+gl-1,2:4) - data_(ii,2:4);
    end
end


contact_data = data_(contact_inds,:);

% process data
pressure_offset= zeros(1,8); % mean(data_top(1:10,5:12),1); % average first 10 samples
pressure_readings = contact_data(:,5:12) - pressure_offset;
sensor_angles = [contact_data(:,33), contact_data(:,34)]; % CHOOSE
...
fingertip_forces = zeros(size(ATI_forces));
for ii=1:size(fingertip_forces,1)
...
%     if (sensor_angles(ii,2)<-120) %||(sensor_angles_top(ii,2)>-60)
%         idxs = [idxs, ii];
...
% 
% 
% pressure_readings(idxs,:) = [];
% contact_data(idxs,:) = [];
% sensor_angles(idxs,:) = [];
% ATI_forces(idxs,:) = [];
% fingertip_forces(idxs,:) = [];

%% Initial plot of data

%pitch and roll angles for each contact point
figure(1); clf; 
...
%forces on fingertip. Should be the same magnitude as ATI forces 
...
%pressure sensor from 8 sensors
subplot(4,1,4);
...
%a lot of times failing senosrs will just be a unifrom distribution instead
%of something more normal distribution looking
...
sgtitle('Distributions of Pressures for Fz > 1.0');
...
angle_min = round(min(sensor_angles),2,"significant");
angle_max= round(max(sensor_angles),3,"significant");
% angle_min=[ -45 -135];
% angle_max=[ 45 45];

% Normalize data, making sure it goes from 0 to 1 to account for rounding
% errors, %different normalization for each pressure sensor channel
