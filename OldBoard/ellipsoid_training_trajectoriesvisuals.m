...
%% set up data saving
save_data = 1;
savename = 'trajectories/ellipsoid_newgantry_trainingv1.csv';
%% generate asterisk pattern at origin

% asterisk parameters
dx = 1; % distance in mm between points along each ray of the asterisk
N1 = 3; % number of pts along each ray of the asterisk (not including center point)
N2 = 8; % number of equally spaced rays of the asterisk

dz0 = 0; % initial compression distance for contact
dz = -0.5; % distance in mm between stacked asterisk patterns
N3 = 4; %3; % number of stacked asterisk patterns in the group
...
ati_nominal = [0,0,12.6 + 15]';  %12.6 is from the point of rotation to top of bare ati / 15 is height of pedestal
...
azimuth_range = [-pi, pi]; % centered at zero
polar_range = [0, -pi/6] + pi; % centered at pi
ma = 20-1; % number of azimuthal angles
...
y = a*r*sin(t).*cos(p); y = y'; 
x = b*r*sin(t).*sin(p); x = x';
z = c*r*cos(t); z = z';
sensor_surface = [];
for ii = 1:1:n
...
clf
figure(1);
plot3(sensor_surface(:,1),sensor_surface(:,2),sensor_surface(:,3),'r.'); hold on;
...
plot3(sensor_surface(:,1),sensor_surface(:,2),sensor_surface(:,3),'.','Color',[0 0.4470 0.7410]	); 
